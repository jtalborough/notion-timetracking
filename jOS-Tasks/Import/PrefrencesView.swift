...
struct PreferencesView: View {
    ...
    func saveToKeychain() {
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            {
                keychain.save(apiKeyData, service: "jOS-Tasks", account: "apiKey")
            }
            
            if   let timeTrackingDatabaseId = self.globalSettings.TimeTrackingDatatbaseId.data(using: .utf8)
            {
                keychain.save(timeTrackingDatabaseId, service: "jOS-Tasks", account: "timeTrackingDatabaseId")
            }
            if let taskDatabaseId = self.globalSettings.TaskDatatbaseId.data(using: .utf8)
            {
                keychain.save(taskDatabaseId, service: "jOS-Tasks", account: "taskDatabaseId")
            }
